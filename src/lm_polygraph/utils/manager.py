...
class UEManager:
    ...
    def _process(self, iterable_data, batch_callback):
        ...
        for batch_i, (inp_texts, target_texts, labels) in enumerate(iterable_data):
            ...
            if labels is not None:
                self.stats["batch_labels"] += labels
                batch_stats["batch_labels"] = labels
    ...
    def __call__(self) -> Dict[Tuple[str, str, str, str], float]:
        ...
        def fn_on_batch_callback(
            ...
            if "batch_labels" in batch_stats and batch_stats["batch_labels"] is not None:
                labels = batch_stats["batch_labels"]
                
                log.info("-" * 100)
                log.info(f"AMBIGUITY LABELS: {labels}")
                log.info("-" * 100)
    ...
    def load(
        ...
        """
        Loads UEManager from the specified path.
        """
