...
class HiddenStateCollector(StatCalculator):
    """
    ...
    * embeddings from the model
    ...
    def __init__(self, 
        ...
        self._batch_counter = 0  # === NEW: track batch order ===
        if save_hidden:
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            if self.output_hidden_states:
                ...
                # === NEW (multi-model compatibility): extract hidden states safely ===

                hs_list = None
                if hasattr(out, "hidden_states") and out.hidden_states:
                    hs = out.hidden_states
                    if isinstance(hs, (list, tuple)) and len(hs) > 0:
                        if isinstance(hs[0], (list, tuple)) and isinstance(hs[0][0], torch.Tensor):
                            # Multi-step structure: concatenate each layer across steps
                            num_layers = len(hs[0])
                            layer_wise = []
                            for layer_idx in range(num_layers):
                                layer_cat = torch.cat(
                                    [step[layer_idx] for step in hs], dim=1
                                )  # [B, input_len + gen_len, H]
                                layer_wise.append(layer_cat)
                            hs_list = torch.stack(layer_wise)  # [L, B, full_seq_len, H]

                        elif isinstance(hs[0], torch.Tensor):
                            # Direct per-layer structure
                            hs_list = torch.stack(list(hs))  # [L, B, full_seq_len, H]
                else:
                    print("[Warning] Model can't provide hidden states or not decoder-only.")
                    hs_list = None

                if hs_list is not None:
                    num_layers = hs_list.shape[0]
                    input_len = batch["input_ids"].shape[1] # equals to len(hs[0][0][0])
                    ...
                    for layer_idx in range(num_layers):
                        # layer_h: [B, full_seq_len, H]
                        layer_h = hs_list[layer_idx]
                        seq_len = layer_h.shape[1]
                        hidden_dim = layer_h.shape[-1]

                        input_h = layer_h[:, :input_len, :] # [B, input_len, H], full_seq_len = input_len + gen_len
                        output_h = layer_h[:, input_len:, :] if seq_len > input_len else layer_h[:, :0, :]  # [B, gen_len, H]

                        if output_h.shape[1] == 0:
                            emb_first_output = torch.zeros(hidden_dim, device=layer_h.device)
                            emb_avg_output  = torch.zeros(hidden_dim, device=layer_h.device)
                            emb_last_output = torch.zeros(hidden_dim, device=layer_h.device)
                        else:
                            emb_first_output = output_h[:, 0, :].mean(dim=0)    # when batch size = 1, this is just output_h[0, 0, :]
                            emb_avg_output  = output_h.mean(dim=1).mean(dim=0)
                            emb_last_output = output_h[:, -1, :].mean(dim=0)   

                        if input_h.shape[1] == 0:
                            emb_last_input = torch.zeros(hidden_dim, device=layer_h.device)
                            emb_avg_input  = torch.zeros(hidden_dim, device=layer_h.device)
                        else:
                            # padding_side='left'，but when batch size = 1, there is no padding, so the attention mask is all ones
                            # mask = batch["attention_mask"].to(layer_h.device)     # [B, input_len]
                            # mask = mask[:, :, None].float()                        # [B, input_len, 1]
                            # denom = mask.sum(dim=1, keepdim=True).clamp(min=1.0)  # [B, 1, 1]
                            # emb_avg_input = (input_h * mask).sum(dim=1) / denom   # [B, H]
                            # emb_avg_input = emb_avg_input.mean(dim=0)             # [H]

                            emb_avg_input = input_h.mean(dim=1).mean(dim=0) # when batch size = 1, no need to consider mask
                            emb_last_input = input_h[:, -1, :].mean(dim=0)        # [H]

                        save_data["hidden_summaries"][f"layer_{layer_idx}"] = {
                            "input_last":  emb_last_input.detach().cpu(),
                            "output_first":emb_first_output.detach().cpu(),
                            "output_avg":  emb_avg_output.detach().cpu(),
                            "input_avg":   emb_avg_input.detach().cpu(),
                            "output_last": emb_last_output.detach().cpu(),
                        }

                    if self.save_hidden:
                        batch_hash = str(uuid.uuid4().hex)[:8]
                        file_name = f"hidden_summary_batch{self._batch_counter:05d}_{batch_hash}.pt"
                        file_path = os.path.join(self.save_dir, file_name)
                        torch.save(save_data, file_path)
                        print(f"[HiddenStateCollector] Saved hidden summaries for batch {self._batch_counter} -> {file_path}")
