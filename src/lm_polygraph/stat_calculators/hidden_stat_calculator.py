...
class HiddenStateCalculator(StatCalculator):
    """
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            if self.output_hidden_states:
                    ...
                    if isinstance(hs, (list, tuple)) and len(hs) > 0:
                        if isinstance(hs[0], (list, tuple)) and isinstance(hs[0][0], torch.Tensor):
                            ...
                            for layer_idx in range(num_layers):
                                layer_cat = torch.cat([step[layer_idx] for step in hs], dim=1)
                ...
                layer_idx = self.target_layer if self.target_layer >= 0 else num_layers - 1
                layer_idx = min(max(layer_idx, 0), num_layers - 1) 

                layer_h = hs_list[layer_idx]  # [B, full_seq_len, H]
                input_len = batch["input_ids"].shape[1]
                input_last = layer_h[:, input_len - 1, :].cpu().detach().numpy()  # [B, H]
